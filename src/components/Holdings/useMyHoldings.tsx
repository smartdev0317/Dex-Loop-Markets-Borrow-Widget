...
const useMyHoldings = () => {
  ...
  const dataSource: DATASOURCE[] = [...tokensListWithuusdPairs, { token: UUSD, pair: UUSD, lp: UUSD }, ...ibc].map((uusdItem) => {
    ...
    const calPrice = ibcList[tokenInfo?.symbol] ? prices[tokenInfo?.symbol] : price
    ...
    return {
      ...
      name: ibcList[tokenInfo?.symbol] ? ibcList[tokenInfo?.symbol]?.name : tokenInfo?.name ?? "",
      token: token,
      pair,
      lpToken: lp,
      balance: balance ?? "0",
