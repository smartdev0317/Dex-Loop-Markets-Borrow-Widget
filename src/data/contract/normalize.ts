...
import { terraswapPairs } from "constant/terraswapPairs"
...
export const terraSwapPairsQuery = selector({
  key: 'terraSwapPairs',
  // get: () => terraswapPairs
  get: async () => {
    // const response = await axios.get('https://api.terraswap.io/pairs');
    try {
      const response = await axios.get('https://assets.terra.money/cw20/pairs.dex.json');
